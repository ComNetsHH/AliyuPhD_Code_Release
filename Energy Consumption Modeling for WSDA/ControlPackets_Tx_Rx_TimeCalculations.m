...
DIFS = 0.00005; %50e-06;
delta= 0.00000247; %1e-06;     %propagation delay(s) from application scenario (largest distance to destination is 742.04m)

payload = 864;          %DIO (108 bytes) 864 bits packet in bits;            %da0 (100B) or 800 bits 
                        %NS packet= NA pckt = 946 bits i.e Neighbor solicitation and Advertisement pckts                          

mac_header = 1e-6;  %272;           %bits (34B)
phy_header = 1e-6;  %128;           %bits (16B)
ack_p = 112;                %bits (14B)
packet = payload + mac_header; %+ phy_header;
%ACK = ack_p + phy_header;

%*********Data packet and ACK time
T_data = (packet/R_data);  %+ (phy_header/R_phy); Cntrl msg payload includes headers 
